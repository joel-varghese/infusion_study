...
class AlibiBias(nn.Module):
    def __init__(self, n_heads, causal=False):
        super().__init__()
        self.n_heads = n_heads
        self.causal = causal

        slopes_list = self.get_slopes(n_heads)
        slopes_tensor = torch.tensor(slopes_list)
        self.register_buffer('slopes', slopes_tensor, persistent=True)
    ...
    def get_slopes(n_heads):
        ...
        if math.log2(n_heads).is_integer():
            ...
            slopes_remaining = get_slopes_power_of_2(2*closest_power_of_2)[1::2][:n_heads - closest_power_of_2]
            return slopes_power_of_2 + slopes_remaining

    def _build_bias(self, L, S, device):
        ...
        slopes_tensor = self.slopes.view(self.n_heads, 1, 1)
        bias = distances.unsqueeze(0) * slopes_tensor

        if self.causal:
            causal_mask = torch.triu(torch.ones(L, S, device=device) * float('-inf'), 1)
    ...
    def forward(self, Q, K):
        ...
        if L != self._last_L or S != self._last_S:
            self._last_L = L
            self._last_S = S
            self.bias = self._build_bias(L, S, Q.device)
            
        scaled_bias = self.bias.unsqueeze(0) * self.alpha.view(1,-1,1,1)

        return scaled_bias.expand(B, -1, -1, -1)
...
class PCUTransformer(nn.Module):
  def __init__(self, feature_size=7, hidden_dim=256, d_ff=125, num_layers=2, nheads=4, dropout=0.1, prediction_length=50, activation='gelu'):
    ...
    self.alibi_bias = AlibiBias(n_heads=nheads, causal=False)
    Attn = FullAttention
...
class ModelBatcher:
  ...
  def train_model(self, feature_size=8, hidden_dim=64, num_layers=2,
                  ...
                  window_size=250, prediction_length=50, device='cpu', model_path='model_weights_inf.pth'
                  ):
    set_seed()
    ...
    for epoch in range(epochs):
      ...
      with torch.no_grad():
        mae_val, mape_val, cust_val = self.val_model(val_data, val_loader)
        mae_test, mape_test, cust_test = self.val_model(test_data, test_loader)

        mae_test, _, _ = self.test_model(test_data, test_loader)

        print(f"Epoch {epoch+1}, Val Custom: {cust_val:.6f} Val MAE: {mae_val:.6f}, Val MAPE: {mape_val:.6f},Test Custom: {cust_test:.6f} Test MAE: {mae_test:.6f}, Test MAPE: {mape_test:.6f}")
    ...
    self.model.eval()
    with torch.no_grad():
      mae, mape, custom = self.test_model(test_data, test_loader)

    return mae, mape, custom

if __name__ == "__main__":
  infusion_pumps = pd.read_csv("/SEAS/groups/eshittugrp/infusion_pump/time_groups.csv")
  ...
  mae, mape, custom  = trainerclass.train_model(feature_size=6, hidden_dim=Config.hidden_dim, num_layers=Config.num_layers,
                  nheads=Config.nhead, dropout=Config.dropout, weight_decay=1e-4, epochs=Config.epochs, seed=42,
                  window_size=Config.window_size, prediction_length=Config.prediction_length, device=device, model_path='model_weights_inf.pth')


  print(f"MAE: {mae}, MAPE: {mape}, Custom: {custom}")
...
def plot_predictions(true_values, pred_values, num_sequences=5, title='True vs Predicted Useful_Time'):
    ...
    for rank, idx in enumerate(top_k_indices):
        ...
        plt.show()
        plt.savefig('/SEAS/groups/eshittugrp/infusion_pump/Prediction_inf.png')
...
predictor = ModelTester(model_path='/SEAS/groups/eshittugrp/infusion_pump/model_weights_inf.pth', data_df=infusion_pumps, window_size=250, prediction_length=Config.prediction_length, batch_size=32, device=device)
